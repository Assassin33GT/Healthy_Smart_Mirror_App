import 'package:flutter/material.dart';
  ...
  class _ControlMirrorState extends State<ControlMirror> {
    final TextEditingController ipController = TextEditingController(text: '192.168.1.50');
    ...
    Future<void> sendCommand(String action, {Map<String, dynamic>? payload}) async {
      ...
      final url = Uri.parse('http://$ip:8080/remote').replace(queryParameters: queryParams);
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          _showSnackBar("Failed: ${response.statusCode} - ${response.reasonPhrase} - ${response.body}");
        }
      } catch (e) {
    ...
    Widget build(BuildContext context) {
      return Scaffold(
        appBar: AppBar(title: const Text("Magic Mirror Controller")),
        body: Padding(
            ...
            child: Column(
              children: [
                TextFormField(
                  ...
                  validator: (value) =>
                      value == null || value.isEmpty ? "Please enter an IP address" : null,
                ),
